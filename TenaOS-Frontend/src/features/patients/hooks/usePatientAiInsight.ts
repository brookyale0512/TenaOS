...
export function usePatientAiInsight(patientUuid: string | undefined) {
  ...
  const mutate = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await tenaAgentClient.post<PatientInsightTrace>(`/insights/patient/${patientUuid}`, {
        workflow: "patient-chart-insight",
      });
      setData(data);
      const tenaAgentBase = (tenaAgentClient.defaults.baseURL ?? "/agent-api").replace(/\/$/, "");
      const source = new EventSource(`${tenaAgentBase}/insights/${data.traceId}/events`);
