...
export function useTextScribe(patientUuid: string) {
  ...
  const processText = useCallback(async (noteText: string, language: "english" | "amharic" = "english") => {
    ...
    setTrace(undefined);
    try {
      const { data: started } = await tenaAgentClient.post<ScribeRunTrace>("/scribe/process_text_trace", { noteText, patientUuid, language });
      setTrace(started);
      const tenaAgentBase = (tenaAgentClient.defaults.baseURL ?? "/agent-api").replace(/\/$/, "");
      await new Promise<void>((resolve, reject) => {
        const source = new EventSource(`${tenaAgentBase}/scribe/${started.traceId}/events`);
  ...
  const confirmNote = useCallback(async ({
    ...
    try {
      ...
      await tenaAgentClient.post("/scribe/confirm_text", {
        patientUuid,
        visitUuid,
        locationUuid,
