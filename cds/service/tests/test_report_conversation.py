...
from cds_service.ciel import ConceptNotFoundError  # noqa: E402
from cds_service.openmrs_reader import FilterSpec  # noqa: E402
...
class FakeUnhealthyVllm:
    ...
    def chat(self, *args: Any, **kwargs: Any) -> Any:
        raise RuntimeError("vLLM should not be called when unhealthy")


class FakeHealthyVllm:
    def __init__(self, contents: list[Any]) -> None:
        self.contents = list(contents)

    def health(self) -> Any:
        return FakeVllmStatus()
...
class _DriverTestBase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.driver = ReportConversationDriver(
            ...
            ciel=self.ciel,  # type: ignore[arg-type]
            loop=self.loop,
            vllm=FakeUnhealthyVllm(),  # default; tests can swap
        )
    ...
    def _swap_vllm(self, vllm: Any) -> None:
        self.driver = ReportConversationDriver(
            store=self.store,
            ciel=self.ciel,  # type: ignore[arg-type]
            loop=self.loop,
...
class KickoffAndNameTurnTests(_DriverTestBase):
    ...
    def test_name_turn_records_user_message_and_advances(self) -> None:
        # Use a healthy VLLM so the auto-run after naming is exercised; we
        # don't care about its tool calls here, just that the name is set.
        self._swap_vllm(FakeHealthyVllm(["Plan: Cough count last quarter\nName: Cough Count\nType: count", "stopping"]))
        draft = self._new_draft()
...
class AgentLoopTests(_DriverTestBase):
    def test_count_happy_path_via_agent_loop(self) -> None:
        # The fake VLLM returns the tool-call script that produces a count report.
        ...
        self._swap_vllm(FakeHealthyVllm(canned))
