...
from cds_service.ciel import ConceptNotFoundError, SeedHit  # noqa: E402
...
class FakeVllm:
    ...
    def chat(self, *args: Any, **kwargs: Any) -> Any:
        raise RuntimeError("vLLM should not be called in tests")
...
class _DriverTestBase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.loop = FormBuilderToolLoop(
            ...
            vllm=FakeVllm(),  # type: ignore[arg-type]
...
- HIV status
"""
        healthy_vllm = FakeHealthyVllm(_agent_build_messages([
        ...
        self.driver = FormConversationDriver(
            ...
            ciel=self.ciel,  # type: ignore[arg-type]
            loop=self.loop,
            vllm=healthy_vllm,  # type: ignore[arg-type]
        )
...
"""
        self.driver = FormConversationDriver(
            ...
            ciel=self.ciel,  # type: ignore[arg-type]
            loop=self.loop,
            vllm=FakeHealthyVllm(_agent_build_messages([
                {"op": "add_section", "sectionId": "vitals", "label": "Vitals"},
                {"op": "add_field", "sectionId": "vitals", "conceptId": "5089", "label": "Weight"},
...
class AgentSafetyBoundaryTests(_DriverTestBase):
    ...
    def _swap_driver(self, vllm: Any) -> None:
        self.driver = FormConversationDriver(
            store=self.store,
            ciel=self.ciel,  # type: ignore[arg-type]
            loop=self.loop,
    ...
    def test_padded_uuid_in_agent_add_field_is_normalized(self) -> None:
        ...
        padded = "5089" + "A" * 32
        self._swap_driver(FakeHealthyVllm(_agent_build_messages([
            {"op": "add_section", "sectionId": "vitals", "label": "Vitals"},
    ...
    def test_final_message_count_matches_actual_basket(self) -> None:
        ...
        self._swap_driver(FakeHealthyVllm([
            "Brainstorm",
...
class CompactToolResultTests(unittest.TestCase):
    ...
    def test_schema_in_tool_result_is_collapsed_to_counts(self) -> None:
        from cds_service.form_conversation import _compact_tool_result  # noqa: WPS433
