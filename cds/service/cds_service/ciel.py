...
`/var/www/ClinicDx_backend/CIEL/ciel_search/`. This client imports it via
`sys.path` injection (configurable through `CDS_CIEL_REPO_ROOT`) and falls
back to a thin direct-SQLite reader if the package is unavailable so the
CDS service can boot in restricted environments.
...
class CielClient:
    ...
    the CDS service can construct a `CielClient` even when the CIEL repo is
    ...
    def availability_detail(self) -> dict[str, Any]:
        ...
        return {
            ...
            "qdrantUrl": self.settings.qdrant_url,
            "qdrantCollection": self.settings.qdrant_collection,
    ...
    def _ensure_service(self) -> Any:
        ...
        ciel_pkg_root = repo_root / "CIEL"
        if str(ciel_pkg_root) not in sys.path:
            sys.path.insert(0, str(ciel_pkg_root))
        try:
    ...
    def _maybe_build_qdrant_search(self) -> Any | None:
        if not self.settings.qdrant_url:
            return None
        try:
            from ciel_search import QdrantHybridSearcher  # type: ignore
        except Exception:
            return None
        try:
