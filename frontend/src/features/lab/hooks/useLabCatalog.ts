...
export function useLabCatalog() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await cdsClient.get<{ catalog: LabCatalog }>("/labs/catalog");
      return data.catalog;
    },
    staleTime: 30_000,
  });
...
export function useAddLabTest() {
  ...
  return useMutation({
    mutationFn: async (description: string): Promise<AddLabTestResult> => {
      const { data } = await cdsClient.post<AddLabTestResult>("/labs/catalog/add", { description });
      return data;
    },
    onSuccess: (result) => {
...
export function useConfirmLabTest() {
  ...
  return useMutation({
    mutationFn: async (candidate: LabCatalogCandidate): Promise<AddLabTestResult> => {
      const { data } = await cdsClient.post<AddLabTestResult>("/labs/catalog/confirm", {
...
export function useRemoveLabTest() {
  ...
  return useMutation({
    mutationFn: async (entryUuid: string) => {
      await cdsClient.post(`/labs/catalog/${entryUuid}/remove`, {});
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["labs-catalog"] });
