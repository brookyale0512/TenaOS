...
interface WorkspaceHealth {
  cdsServiceReady: boolean;
  cielReady: boolean;
  gemmaReady: boolean;
  cdsServiceStale: boolean;
  isPending: boolean;
  isError: boolean;
  refetch: () => void;
}
...
export function FormBuilderWorkspace() {
  ...
  const cdsServiceStale = cdsHealth.isSuccess && cdsServiceReady && !cdsHealth.data?.ciel;
  ...
  const health: WorkspaceHealth = {
    ...
    gemmaReady,
    cdsServiceStale,
    isPending: cdsHealth.isPending,
    isError: cdsHealth.isError,
    refetch: () => cdsHealth.refetch(),
  };
...
function HealthErrors({ health }: { health: WorkspaceHealth }) {
  return (
    <>
      {health.cdsServiceStale && (
        <ErrorState
          title="CDS service is running an older build"
          description="The CDS service does not expose the form-builder endpoints. Restart the CDS container."
        />
      )}
      {!health.cdsServiceReady && !health.isPending && !health.isError && (
        <ErrorState
          title="CDS service is not reachable"
          description="The form builder requires the CDS service at /cds-api to be running."
        />
      )}
      {health.isError && (
        <ErrorState
          title="CDS service is offline"
          description="The form builder requires the CDS service, Gemma 4, and CIEL to be reachable."
          onRetry={health.refetch}
        />
      )}
      {health.cdsServiceReady && !health.cielReady && !health.cdsServiceStale && (
        <ErrorState
          title="CIEL terminology is not reachable"
          description="The CDS service cannot reach the CIEL search store. The form builder needs CIEL to pick concepts."
        />
      )}
      {health.cdsServiceReady && health.cielReady && !health.gemmaReady && (
        <ErrorState
