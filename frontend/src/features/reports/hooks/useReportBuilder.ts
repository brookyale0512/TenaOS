...
export function useCreateReportDraft() {
  ...
  return useMutation({
    mutationFn: async (payload: CreateReportDraftPayload = {}) => {
      const { data } = await cdsClient.post<ReportDraft>(`${REPORTS_API}/drafts`, payload);
      return data;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["cds", "report-drafts"] }),
...
export function useReportDraftList() {
  return useQuery({
    queryKey: ["cds", "report-drafts"],
    queryFn: async () => {
      const { data } = await cdsClient.get(`${REPORTS_API}/drafts`);
...
export function usePublishedReportList() {
  return useQuery({
    queryKey: ["cds", "report-drafts", "published"],
    queryFn: async () => {
      const { data } = await cdsClient.get(`${REPORTS_API}/drafts`, { params: { published: "true" } });
      return (data.drafts ?? []) as ReportDraft[];
    },
    staleTime: 30 * 1000,
  });
...
export function useReportDraft(draftId: string | undefined) {
  return useQuery({
    queryKey: ["cds", "report-drafts", draftId],
    queryFn: async (): Promise<ReportDraft> => {
      const { data } = await cdsClient.get<ReportDraft>(`${REPORTS_API}/drafts/${draftId}`);
      return data;
    },
    enabled: !!draftId,
...
export function useReportResult(draftId: string | undefined) {
  return useQuery({
    queryKey: ["cds", "report-drafts", draftId, "result"],
    queryFn: async () => {
      const { data } = await cdsClient.get(`${REPORTS_API}/drafts/${draftId}/result`);
...
export function useSendReportMessage(draftId: string | undefined) {
  ...
  return useMutation({
    mutationFn: async (message: string) => {
      ...
      const { data } = await cdsClient.post(`${REPORTS_API}/drafts/${draftId}/messages`, { message });
      return data;
    },
    onSuccess: () => {
      if (!draftId) return;
      qc.invalidateQueries({ queryKey: ["cds", "report-drafts", draftId] });
    },
...
export function useApplyReportAction(draftId: string | undefined) {
  ...
  return useMutation({
    mutationFn: async (action: ReportAction) => {
      if (!draftId) throw new Error("draftId is required");
      const { data } = await cdsClient.post(`${REPORTS_API}/drafts/${draftId}/actions`, action);
      return data;
    },
    onSuccess: () => {
      if (!draftId) return;
...
export function usePublishReport(draftId: string | undefined) {
  ...
  return useMutation({
    mutationFn: async (published: boolean) => {
      if (!draftId) throw new Error("draftId is required");
      const { data } = await cdsClient.post<ReportDraft>(`${REPORTS_API}/drafts/${draftId}/actions`, {
    ...
    onSuccess: (data) => {
      qc.invalidateQueries({ queryKey: ["cds", "report-drafts"] });
      qc.invalidateQueries({ queryKey: ["cds", "report-drafts", "published"] });
      qc.invalidateQueries({ queryKey: ["cds", "report-drafts", draftId] });
...
export function useDeleteReportDraft() {
  ...
  return useMutation({
    mutationFn: async (draftId: string) => {
      await cdsClient.delete(`${REPORTS_API}/drafts/${draftId}`);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["cds", "report-drafts"] });
      qc.invalidateQueries({ queryKey: ["cds", "report-drafts", "published"] });
...
export function useApplyReportOperations(draftId: string | undefined) {
  ...
  return useMutation({
    mutationFn: async (operations: ReportOperation[]) => {
      if (!draftId) throw new Error("draftId is required");
      const { data } = await cdsClient.post(`${REPORTS_API}/drafts/${draftId}/operations`, { operations });
      return data;
    },
    onSuccess: () => {
      if (!draftId) return;
...
export function useRunReport(draftId: string | undefined) {
  ...
  return useMutation({
    mutationFn: async () => {
      if (!draftId) throw new Error("draftId is required");
      const { data } = await cdsClient.post(`${REPORTS_API}/drafts/${draftId}/run`, {});
      return data;
    },
    onSuccess: () => {
      if (!draftId) return;
      qc.invalidateQueries({ queryKey: ["cds", "report-drafts", draftId] });
      qc.invalidateQueries({ queryKey: ["cds", "report-drafts", draftId, "result"] });
    },
...
export function useReportDraftEvents(draftId: string | undefined): {
  ...
  useEffect(() => {
    ...
    const loadInitial = async () => {
      try {
        const { data } = await cdsClient.get(`${REPORTS_API}/drafts/${draftId}/events`);
        if (aborted) return;
