...
_MATERIAL_SYSTEM_PROMPT = material_system()
...
class PatientMaterialLoop:
    ...
    def run(self, trace: InsightTrace, context: Any) -> dict[str, Any]:
        ...
        if self.trace_store is not None:
            attach_store(
                trace,
                self.trace_store,
                summary=f"Material trace {getattr(trace, 'trace_id', 'unknown')}",
                context={"patientUuid": getattr(trace, "patient_uuid", None), "traceId": getattr(trace, "trace_id", None)},
            )
        ...
        if not llm_healthy:
            trace.add("model_fallback", "TenaOS-LLM unavailable", "Cannot generate patient material without Gemma 4.")
    ...
    def _call_llm(
        ...
        try:
            ...
            trace.add("llm_error", f"LLM error (step {turn + 1})", str(exc))
            return None
