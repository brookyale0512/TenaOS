...
from .agent_prompts import apply_tool_description_overlay, form_brainstorm_system, form_tool_system
    ...
    *,
    ...
    if _basket_field_count(latest) == 0:
        recovery = ask_gemma_for_recovery_operations(
            llm=llm, store=store, draft_id=draft.draft_id, request=request,
        )
        if recovery:
...
def _call_gemma_tool_turn(
    *,
    ...
    response = llm.chat(
        ...
        temperature=temperature,
        max_tokens=max_tokens,
        tools=apply_tool_description_overlay("form_builder", FORM_OPENAI_TOOLS),
        tool_choice="auto",
    )
    store.append_event(
        draft_id,
...
def _call_gemma_json(
    *,
    ...
    Returns None for two distinct failure modes:
      - model unreachable / timeout → llm.chat raises
